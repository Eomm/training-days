...
- [ ] Task 1 — Install missing backend dependencies (AC: 6)
  - [ ] `npm install @fastify/swagger @fastify/swagger-ui @fastify/env fastify-plugin --workspace=backend`
  - [ ] Verify `backend/package.json` lists all required packages: `fastify@5`, `@fastify/cors`, `@fastify/helmet`, `@fastify/rate-limit`, `@fastify/swagger`, `@fastify/swagger-ui`, `@fastify/env`, `fastify-plugin`
  - [ ] `fastify-plugin` is installed now but only used starting Story 1.4 (DB plugin). Import it in `app.ts` at that point, not here.

- [ ] Task 2 — Create `backend/src/app.ts` factory (AC: 4, 6)
  - [ ] Export an async `build(opts?)` function that creates a Fastify instance and returns it — **never calls `listen()`**
  - [ ] Register all plugins in order: helmet → cors → rate-limit → swagger → swagger-ui → routes
  - [ ] Register `GET /health` route (inline or via `health.route.ts`)
  - [ ] Log level: read from env (`LOG_LEVEL`) or default to `'info'`; accept `logger` override via opts for test isolation

- [ ] Task 3 — Create `backend/src/server.ts` entrypoint (AC: 1, 5)
  - [ ] Call `build()` then `app.listen({ port: Number(process.env.PORT ?? 3000), host: '0.0.0.0' })`
  - [ ] This is the **only** file that calls `listen()`
  - [ ] No `--env-file` flag needed — `@fastify/env` loads and validates env inside `build()`

- [ ] Task 4 — Create `backend/src/routes/health.route.ts` (AC: 2)
  - [ ] `GET /health` → `200 OK` with `{ status: 'ok' }` — full JSON Schema on response
  - [ ] Route is **unauthenticated** (no `preHandler` hook)
  - [ ] Route is **exempt from rate limiting** (configure rate-limit to exclude `/health`)
  - [ ] Register in `app.ts` via `fastify.register(healthRoute)`

- [ ] Task 5 — Configure all security plugins (AC: 6)
  - [ ] `@fastify/helmet` — register with defaults (includes CSP, HSTS, X-Frame-Options)
  - [ ] `@fastify/cors` — `origin: process.env.CORS_ORIGIN ?? 'http://localhost:5173'`
  - [ ] `@fastify/rate-limit` — global default (e.g. 100 req/min); exclude `/health`
  - [ ] `@fastify/swagger` — register **before** swagger-ui; openapi 3 spec with `title: 'MotivaTodo API'`, `version: '1.0.0'`
  - [ ] `@fastify/swagger-ui` — `routePrefix: '/documentation'`

- [ ] Task 6 — Update `backend/package.json` scripts (AC: 1, 8)
  - [ ] `"dev": "node --experimental-strip-types --watch src/server.ts"`
  - [ ] `"start": "node dist/server.js"` (production — compiled output)
  - [ ] `"build": "tsc"`
  - [ ] `"test": "node --experimental-strip-types --test test/**/*.test.ts"`

- [ ] Task 7 — Write tests (AC: 3, 8)
  - [ ] Create `backend/test/health.test.ts`
  - [ ] Test 1: `GET /health` → 200 with `{ status: 'ok' }`
  - [ ] Test 2: `GET /documentation` → 200 (Swagger UI is reachable — AC: 3)
  - [ ] Test 3: `GET /nonexistent` → error body has shape `{ statusCode, error, message }` (validates error handler)
  - [ ] Use `app.inject()` — do **not** call `listen()` in test
  - [ ] Call `app.close()` in cleanup (`t.after(() => app.close())`)

- [ ] Task 8 — Update `.env.example` (AC: 7)
  - [ ] Ensure `backend/.env.example` contains all variables: `DATABASE_URL`, `PORT`, `CORS_ORIGIN`, `LOG_LEVEL`
